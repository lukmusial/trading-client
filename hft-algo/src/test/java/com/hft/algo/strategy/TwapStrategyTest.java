...
class TwapStrategyTest {
    ...
    @Test
    void getName_ShouldReturnTWAP() {
        strategy = createStrategy(null);
        assertEquals("TWAP", strategy.getName());
    }

    @Test
    void getDisplayName_WithCustomName_ShouldReturnCustomName() {
        strategy = createStrategy("My TWAP Strategy");
        assertEquals("My TWAP Strategy", strategy.getDisplayName());
    }

    @Test
    void getDisplayName_WithoutCustomName_ShouldReturnTypeName() {
        strategy = createStrategy(null);
        assertEquals("TWAP", strategy.getDisplayName());
    }
    ...
    void onFill_ShouldUpdateProgress() {
        StrategyParameters params = new StrategyParameters()
                ...
                .set("sliceIntervalSeconds", 60L);
        strategy = new TwapStrategy(Set.of(testSymbol), params, null);
    ...
    private TwapStrategy createStrategy(String customName) {
        StrategyParameters params = new StrategyParameters()
                .set("targetQuantity", 1000L)
                .set("durationMinutes", 60L)
                .set("sliceIntervalSeconds", 60L)
                .set("maxParticipationRate", 0.25);
        TwapStrategy s = new TwapStrategy(Set.of(testSymbol), params, customName);
